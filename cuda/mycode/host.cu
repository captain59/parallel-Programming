...
int main(void)
{
	...
	err = cudaMemcpy(d_outKernel_1, h_outKernel_1, size, cudaMemcpyDeviceToHost);
	if(err != cudaSuccess){
	...
	err = cudaMemcpy(d_outKernel_2, h_outKernel_2, size, cudaMemcpyDeviceToHost);
	if(err != cudaSuccess){
	...
	for(int i=0; i < numElements; i++){
		if(fabs(host_output_1 - h_outKernel_1) > errorLimit){
	...
	host_process_kernel_2(host_output_1, host_output_2);
	for(int i=0; i < numElements; i++){
		if(fabs(host_output_2 - h_outKernel_2) > errorLimit){
	...
	host_process_kernel_3(host_output_2, host_output_3);
	for(int i=0; i < numElements; i++){
		if(fabs(host_output_3 - h_outKernel_3) > errorLimit){
