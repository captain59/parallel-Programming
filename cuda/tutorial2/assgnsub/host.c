...
int main(int argc, char const *argv[])
{
	...
	*/
	...
	size_t sizeconv2D = mask2D*sizeof(float);
	...
	float *d_2Dmask = NULL;
	err = cudaMalloc((void **)&d_2Dmask, sizeconv2D);
	if(err != cudaSuccess){
	...
	cudaMemcpy(d_2Dmask, h_2Dmask, sizeconv2D, cudaMemcpyHostToDevice);
	// Output Arrays
	...
	err = cudaGetLastError();
	if(err != cudaSuccess) {
		fprintf(stderr, "Failed to Launch 2D Convolution Kernel: %s\n ", cudaGetErrorString(err));
		exit(EXIT_FAILURE);
	}
	...
	// Verificaiton Result
	float *h_Verification2D = (float*)malloc(sizeArr);
	verify2DResult(h_2Darr, h_2Dmask, h_Verification2D);
	DisplayMatrix2D("Verification 2D Results", h_2Dout, N);
...
void verify2DResult(int *arr, int *mask, int *h_Verification2D) {
	for(int i = 0; i < N; i++) {
		for(int j = 0; j < N; j++) {
			int cnum = 0, id;
			for(int p = i-1; i <= i+1; p++) {
				for(int q = j-1; j<= j+1; q++) {
					if(p>=0 && p<N && q>=0 && q<N) {
						id = p*N + q;
						h_Verification2D[id] = arr[id]*mask[cnum];
					}
					cnum++;
				}
			}
