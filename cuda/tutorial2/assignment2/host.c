...
void DisplayMatrix2D(const char*, int*, int);
...
int main(int argc, char const *argv[])
{
	...
	*/
	...
	dim3 threadsPerBlock(32, 32);
	// Launching Kernel
	convolution2d<<<blocksPerGrid, threadsPerBlock>>>(d_2Darr, d_2Dmask, d_2dresult, N);
...
void DisplayMatrix2D(const char *inp, int *arr, int num) {
	printf("%s\n", inp);
	for(int i = 0; i < num; i++) {
		for(int j = 0; j < num; j++) {
